...
public class BattleManager : MonoBehaviour
{
    ...
    private IEnumerator GetCard(int count)
    {
        ...
        {
            // 抽牌組第一張 放到 手牌第一張
            battleDeck.Add(DeckMamager.instance.deck[0]);
            // 刪除 牌組第一張
            DeckMamager.instance.deck.RemoveAt(0);
            // 抽牌組第一張卡牌物件 放到 手牌 第一張
            handGameObject.Add(DeckMamager.instance.deckGameObject[0]);
            // 刪除 牌組第一張遊戲物件
            DeckMamager.instance.deckGameObject.RemoveAt(0);
            // 等待協程執行結束
    ...
    private IEnumerator MoveCard()
    {
        ...
        {
            card.anchoredPosition = Vector2.Lerp(card.anchoredPosition, new Vector2(0, -275), 0.5f * Time.deltaTime * 30);
